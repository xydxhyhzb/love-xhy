...
if not exist "server.js" (
    echo [信息] 创建服务器文件...
    call :createServerFile 8001 "表白墙管理员后台"
)
...
if %errorlevel% equ 0 (
    ...
    if not exist "server_8081.js" (
        call :createServerFile 8081 "表白墙管理员后台"
        move server_8081.js server.js >nul 2>&1
    )
    echo [信息] 管理员后台将在端口8081运行
    set PORT=8081
) else (
    echo [信息] 端口8001可用
    set PORT=8001
...
echo [成功] 服务器已准备就绪！
echo.
echo [信息] 管理员后台访问地址: http://localhost:%PORT%
echo [信息] 默认登录账号: admin / 123456
...
echo   console.log(''); >> server_%1.js
echo   console.log('默认登录账号: admin'); >> server_%1.js
echo   console.log('默认登录密码: 123456'); >> server_%1.js
echo   console.log(''); >> server_%1.js
echo   '========================================'); >> server_%1.js
echo }); >> server_%1.js
goto :eof